...
def normalize(df):
    ...
    data['Volume'] = min_max_scaler.fit_transform(df.Volume.values.reshape(-1,1))
    data['Close'] = min_max_scaler.fit_transform(df.Close.values.reshape(-1,1))
...
def stock_prediction(symbol: str, start_date: str, end_date=None):
